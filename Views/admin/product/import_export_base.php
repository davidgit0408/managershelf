<?php
    ...
    $categorias = array_unique($categories); 
    $marcas = array_unique($brands); 
    $fabricantes =  array_unique($producers); 
?>
<!DOCTYPE html>
    ...
    <div class="col-md-12 container mt-3">
        <div class="card">
            <div class="card-header border-0" style="background-color: #fc9700;">
                <div class="row align-items-center col-12 ">
                    <div class="col-12 col-md-8 col-sm-8 col-lg-8 col-xl-8 col-xl-8" style="background-color: #fc9700;">
                        <h3 class="mb-0 text-white">Importar Base Produto</h3>
                    </div>                                        
            ...
            <div class="card-body mb-5">
                <h6 class="card-subtitle text-muted mt-2">Baixe o modelo de importação e importe os Base Produto no formato .CSV</h6>
                <hr class="mt-2"/>              
                <form method="post" enctype="multipart/form-data" action="import_csv_base" class="validation-wizard wizard-circle m-t-40" >
                    ...
                    <div class="form-group">
                        ...
                        <div class="file-field">
                            ...
                            <div class="d-flex">
                                ...
                                <button id="enviar" class="btn bg-orange text-white" name="submit" value="Upload" type="submit">Enviar</button>
                            </div>
                        </div>
                    </div>        
                </form>
    ...
    <div class="col-md-12 container mt-3">
        <div class="card">
            <div class="card-header border-0" style="background-color: #fc9700;">
                <div class="row align-items-center col-12 ">
                    <div class="col-12 col-md-8 col-sm-8 col-lg-8 col-xl-8 col-xl-8" style="background-color: #fc9700;">
                        <h3 class="mb-0 text-white">Exportar Base Produto</h3>
                    </div>                                        
                    <div class="col-12 col-md-4 col-sm-4 col-lg-4 col-xl-4 col-xl-4 text-right p-0 mt-1">
                        <a href="bigdata_products"><button class="btn btn bg-white float-end m-1">Base de Produto</button></a>
                    </div>
                </div> 
            </div>
            <div class="card-body wizard-content">
            <h6 class="card-subtitle text-muted mt-2">Exporte os Base Produto filtrando por categoria, marca e/ou fabricante</h6>
            <hr class="mt-2"/>              
            <?php if($error_msg){ ?>
            <div class="alert alert-danger alert-dismissible" role="alert">
                <div class="alert-message">
            ...
            <?php } ?>
                <form method="post" enctype="multipart/form-data" action="<?= base_url('index.php/export_csv_base') ?>" class="row validation-wizard wizard-circle m-t-40" >
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label for="person_type">Filtrar por categoria <span class="danger"></span> </label>
                            <select name="category" class="form-control" required>
                                <option value="all">Todas</option>
                                <?php foreach($categorias as $categoria){foreach($all_categories as $categories){if($categoria == $categories['id'] ){ ?>
                                <option value="<?php echo $categoria ?>"><?php echo $categories['name'] ?></option>
                                <?php }}} ?>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label for="person_type">Filtrar por marca <span class="danger"></span> </label>
                            <select name="brand" class="form-control" required>
                                <option value="all">Todas</option>
                                <?php foreach($marcas as $marca){ if($marca != ''){ ?>
                    ...
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label for="person_type">Filtrar por fabricante <span class="danger"></span> </label>
                            <select name="producer" class="form-control" required>
                                <option value="all">Todas</option>
                                <?php foreach($fabricantes as $fabricante){ if($fabricante != ''){?>
                                <option><?php echo $fabricante ?></option>
                                <?php }} ?>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-12 text-center mt-3">
                        <button type="submit" id="submit-dropzone" class="btn bg-orange text-white">Download</button>
                    </div>
                </form>
            </div>
        </div>
